...
<template>
  <div class="dialog-container">
    <z-header title="对话框" bg="linear-gradient(45deg, #0081ff, #1cbbb4)"></z-header>
    <z-body>
    ...
    <z-dialog :value="defaultFlag" type="confirm" @confirm="defaultFlag=false" @cancel="defaultFlag=false">
...
export default {
  ...
  methods: {
    ...
    showDialogConfirm(){
      Dialog.confirm("今日次数已达上限").then(res=>{
    ...
    showDialogAsync(){
       Dialog.confirm({content:"今日次数已达上限",beforeClose:(action,done)=>{
         if(action=="confirm"){
           setTimeout(()=>{
             done()
           },3000)
